...
- **Gate:** a merchant can see every order, webhook, and dollar; extension completes a partner checkout end-to-end.
  - STATUS 2026-07-04: dashboard half of the gate is MET (5.1/5.2/5.3/5.7 done + deployed). **Remaining (need a design decision or a fresh focused session):** 5.4 extension Privy-auth-in-popup (MV3 popup can't run Privy's web SDK as-is — needs a chrome.identity flow or an auth-relay page; design decision); 5.5 extension-initiated checkout session (the extension must NOT hold a `sk_visby_` secret key — needs a publishable-key-safe or partner-backend-mediated session-creation path; design decision) — extension is also still unpublished, so it can't be tested end-to-end yet; 5.6 merchant payout settlement — SDK side currently has ZERO payout code (buyer funds land in Visby treasury, `merchant_net_usd` is computed+stored but never disbursed). A CRYPTO payout to the already-stored+validated `merchants.merchant_wallet` (USDC 1:1, reusing 4.5's `sendUsdcFromAuthority`) is buildable NOW and NOT onboarding-blocked — best built atomically in a fresh session (money path → build + adversarial-verify + deploy as one unit, to avoid a half-built money path).
