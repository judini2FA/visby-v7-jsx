...
- **Gate:** a merchant can see every order, webhook, and dollar; extension completes a partner checkout end-to-end.
  - STATUS 2026-07-04: dashboard half of the gate is MET (5.1/5.2/5.3/5.7 done + deployed) and 5.4 (extension auth-relay) is done. **Remaining: only 5.6.** 5.5 was reframed by Judah (extension = burner-card autofill on non-partner sites → its real form is the Phase-10 regulated card-issuer, not a partner-checkout session; so 5.5-as-written is obsolete/parked). 5.6 merchant payout settlement — SDK side currently has ZERO payout code (buyer funds land in Visby treasury, `merchant_net_usd` is computed+stored but never disbursed). A CRYPTO payout to the already-stored+validated `merchants.merchant_wallet` (USDC 1:1, reusing 4.5's `sendUsdcFromAuthority`) is buildable NOW and NOT onboarding-blocked — best built atomically in a fresh session (money path → build + adversarial-verify + deploy as one unit, to avoid a half-built money path). NOTE: the extension's live e2e (auth-relay token handoff) needs the unpublished extension loaded in Chrome — a manual Judah step.
