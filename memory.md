...
5. **Civic account (KYC) + Chainalysis account (sanctions screening) — NOW ACTIVE (we're in Phase 6).** These two signups are the ONLY thing blocking Phase 6's fraud core: (a) **Civic** (replaces Persona for KYC) — sign up + get API keys; then I build the Civic adapter (keeping the built fail-closed webhook architecture + sell gates) + fix multi-wallet KYC propagation + flip `NEXT_PUBLIC_KYC_REQUIRED=1` (sell gated, buy open) = blueprint 6.1/6.2/6.3. (b) **Chainalysis** — account + API key; then I build wallet screening before mint/sale/payout (fail-closed on sanctioned hits + admin review queue) = 6.4. I can't build these blind (their real API/webhook shapes differ from Persona and need the account). Neither is in Composio (specialized compliance vendors) → manual signups.
