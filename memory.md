...
5. **Phase 6 fraud core — DECISION 2026-07-04: Civic DROPPED → Stripe Identity for KYC; Chainalysis = FREE sanctions API only (skip the expensive product).** Both blockers now cheap/free: (a) **KYC = Stripe Identity** (~$1.50/verification, first 50 free) — buildable NOW (Stripe already integrated), Judah just enables Identity in the Stripe dashboard (one toggle); then I build verificationSessions + webhook (fail-closed, same architecture) + multi-wallet propagation + flip `NEXT_PUBLIC_KYC_REQUIRED=1` = 6.1/6.2/6.3. (b) **Sanctions screening = Chainalysis FREE Sanctions Screening API** (address vs OFAC SDN list) — Judah grabs a free-tool API key (2-min signup; free oracle is EVM-only so we use the free API for Solana); then I build buyer/seller wallet screening before mint/sale/payout, fail-closed + admin queue = 6.4. The expensive Chainalysis enterprise product (KYT/risk-scoring) is NOT needed for launch. **Persona stub replaced by Stripe Identity; Civic fully retired.**
