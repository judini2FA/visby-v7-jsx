...
5. **Phase 6 fraud core — Stripe Identity KYC is now BUILT (dark); to ENABLE it (Judah, 3 steps):** (1) turn on **Stripe Identity** in the Stripe dashboard (activate the product); (2) add a Stripe **webhook endpoint** for `identity.verification_session.*` events → `https://visby.me/api/kyc/webhook`, then set its signing secret as `STRIPE_IDENTITY_WEBHOOK_SECRET` in Vercel; (3) when ready to gate selling, set `NEXT_PUBLIC_KYC_REQUIRED=1` (buy stays open, sell gated). Cost ~$1.50/verification, first 50 free. Civic fully retired; persona.ts is dead code. **Sanctions screening (6.4) is DEFERRABLE for the closed MVP** (Judah's call: "can we get by without them" → yes for a closed beta); when wanted, it's the Chainalysis FREE Sanctions API (skip the expensive enterprise product) — Judah grabs a free API key, I build the fail-closed screen + admin queue. Attorney sign-off on the non-custodial/KYC posture before real money at scale (standing non-code blocker).
...
9. **4.4 ACH pay-in (built, flag-dark)** — to turn it on: (a) subscribe the Stripe webhook endpoint (visby.me/api/stripe/webhook) to events `payment_intent.payment_failed` AND `payment_intent.canceled` in addition to `payment_intent.succeeded` + `checkout.session.completed` (else ACH failure/cancel handling + single-flight release won't fire); (b) do a live ACH test end-to-end; (c) then set `NEXT_PUBLIC_ACH_ENABLED=1` in Vercel + .env.local. Note the app still runs on the TEST Stripe key — reconcile test vs live mode before real ACH money.
10. **4.9 Stripe Tax (built, flag-dark)** — enable ONLY after: (a) confirm sales-tax nexus + register to collect in the relevant states, (b) enable Stripe Tax in the Stripe dashboard + set product tax codes (optionally `STRIPE_TAX_PRODUCT_CODE` env), then (c) set `STRIPE_TAX_ENABLED=1`. Collecting tax without registration is a liability — stays dark until then.
11. **4.10 1099-K (built, Stripe-native)** — enable Stripe Connect tax-form (1099-K) filing in the Connect dashboard once you have the entity/EIN; Stripe files for bank-payout sellers automatically (Express onboarding already collects their tax info). Crypto-payout (SOL/USDC) sellers are NOT covered — decide if/how Visby issues their 1099s (needs entity + a tax-form path).
...
- **2026-07-04** — **INTERRUPTION RECOVERY + 4.3.** Judah's computer died mid-session while the 4.3 Stripe-Connect-payouts ultracode workflow was running; he asked me to make sure nothing broke and continue. **Assessment: nothing broke** — `git diff --stat` was EMPTY (zero tracked files modified), so the app compiled/ran exactly as the last deploy (`19c5719`). The workflow had left 4.3 **half-built** as untracked files: `src/lib/stripe-connect.ts` + `supabase/migration_seller_connect.sql` + 2 of 3 routes (`connect/onboard`, `connect/status`) were complete, but `connect/refresh/` was an EMPTY dir and — critically — `src/lib/payout.ts` never got the fiat branch (the actual money-path integration). Also noted: `src/app/api/onramp/fulfill/route.ts` was modified by user/linter (kept — added the isBanned ban-freeze via getAuthedContext).
  - **Completed 4.3 myself** (money path, so no delegation): wrote `connect/refresh/route.ts` (POST → refreshConnectStatus, authed+rate-limited); added the ADDITIVE fiat branch to `payout.ts` — new `maybeConnectPayout()` gate fires ONLY when seller `payout_settings.payout_type==='bank'` AND `seller_connect_accounts.payouts_enabled`; **crypto path is byte-unchanged and the default**; any DB miss/error/throw → falls through to crypto; a fiat *transfer* failure returns `{ok:false}` (retryable) and is NEVER silently downgraded to crypto (double-pay guard). Idempotency = `connect-payout:${order.id}` on stripe.transfers.create.
  - **Reconciled with existing bank plumbing**: `payout_settings.stripe_account_id` + `/api/payout/test` already existed (raw-acct-paste UX). Reworked `/api/payout` POST to resolve the bank acct id server-side from `seller_connect_accounts` (seller never pastes acct_ again); bank step-up now binds the constant `'connect'` on BOTH client+server (one connected account per wallet, step-up still dormant until STEP_UP_ENFORCED=1). Sonnet subagent reworking `payout-settings.tsx` bank UI → real Connect onboarding button + status (replaces the paste input).
  - Migration `seller_connect_accounts_4_3` APPLIED LIVE via Composio (project rwdwzigqtfezbyqkfqfx) — additive table, RLS-enabled service-role-only. tsc clean. Deployed (commit ca24d77) — 4.3 connect endpoints verified live on visby.me (clean 401 unauth). Blueprint 4.3 checked off. See [[launch-integrations-status]].
- **2026-07-04 (cont.)** — **4.4 ACH pay-in DONE, flag-dark** (`NEXT_PUBLIC_ACH_ENABLED`). Buyer pays for an item by bank debit from a 4.1-linked Financial-Connections bank; async settlement (1–3 business days), so NOTHING mints until `payment_intent.succeeded`. I authored the money core myself; Opus-designed, then **4 adversarial ultracode-verify rounds** each of which found real bugs I fixed (a rare pattern — my own fixes kept introducing subtler edges, so each round was worth it):
  - Files: NEW `/api/stripe/ach-payment-intent` (authed+ban+rate-limited; derives a us_bank_account PM from the FC account via `financial_connections_account`, attaches to the buyer's stripe_customer, server-confirms w/ online mandate → `processing`; metadata.pay_method='ach'); webhook `payment_intent.{succeeded,payment_failed,canceled}` ACH branches; `src/lib/ach-payins.ts` + `migration_ach_payins.sql` (LIVE, +14d-expiry ALTER LIVE); checkout-modal ACH tab + `ach_processing` state (does NOT call onSuccess — nothing minted); `payable-tokens` achEnabled() gate; `email-templates.achPaymentFailedBuyer`. `/api/payout` server-resolves the acct id, etc. all unchanged.
  - **Round 1 (5 lenses)** found: (a) double-debit across the settle window (Stripe idempotency key is only 24h, ACH takes days) → built durable single-flight `ach_payins` (partial-unique on (item,buyer) WHERE processing); (b) sold-away detector `owner!=buyer && !is_listed` false-refunds a seller-paused/admin-delisted item → now requires a THIRD-PARTY owner (≠buyer AND ≠seller from PI metadata).
  - **Round 2 (2 lenses)** found: my self-heal DELETE reclaimed ANY expired processing row incl. live attached ones → a >5-day settle + resubmit = 2nd debit. Fixed: self-heal only deletes orphans (`payment_intent_id IS NULL`); widened expiry 5→14 days (past max ACH return window).
  - **Round 3 (1 agent)** found: no `payment_intent.canceled` handler → a canceled ACH left the claim stuck 'processing' forever (pi_id non-null, orphan-heal can't touch it) = permanent block → added canceled→mark 'failed'. Plus crash-between-PI-create-and-attach → wrapped `attachAchPayinPi` (can't 500 a valid purchase; PI still fulfills via metadata). Added a redundant-debit guard (refund a 2nd ACH for an already-delivered item).
  - **Round 4 (1 agent)** found: the redundant-guard's `order.spi !== pi.id` treated "NO order exists" as "duplicate → refund", which would refund an ACH that itself delivered the item with a failed order-write = give it away FREE (loss). Also the reviewer's naive fix would've KEPT genuine crypto-then-ACH duplicates (crypto orders have null spi). Correct fix: refund ONLY when an order EXISTS with a different/null spi (a real different payment); NO order → log for reconcile, do NOT refund; order.spi===pi.id → redelivery no-op.
  - **NET money guarantees:** no mint/transfer before funds clear; no double bank debit (durable single-flight); refund a genuine duplicate/sold-away but NEVER a legit payment; failed/canceled frees the retry slot. tsc clean all rounds; deployed dark.
  - **Judah action items for 4.4** (below): set `NEXT_PUBLIC_ACH_ENABLED=1` after a live ACH test; subscribe the Stripe webhook endpoint to `payment_intent.payment_failed` + `payment_intent.canceled` (currently likely only succeeded + checkout.session.completed); TEST-key mode reconcile before real ACH.
  - Phase 4 remaining: 4.5 multi-currency auto-conversion, 4.6 Moov card reader checkout, 4.9 Stripe Tax (flag, needs Judah nexus), 4.10 1099-K (flag, needs Judah entity).
- **2026-07-04 (cont.)** — **4.5 multi-currency payout DONE.** Discovery that made it clean: `sendUsdcFromAuthority` + `getAuthorityUsdcBalance` ALREADY exist in solana-fund.ts (creates recipient ATA on first receive, clear error if treasury float low) — so USDC payout needs NO swap (net USD is 1:1 USDC). Cross-chain (ETH) still needs a mainnet Li.Fi swap → intentionally deferred. Built: `payout_asset` col on payout_settings (SOL|USDC, migration live) + `maybeUsdcPayout` gate in payout.ts (mirrors the 4.3 `maybeConnectPayout` shape: crypto+USDC → sendUsdcFromAuthority; DB miss/error → null → SOL default; transfer fail → {ok:false} retryable, never downgraded) + /api/payout persists payout_asset + payout-settings.tsx SOL/USDC "Receive as" toggle (USDC = "steady $1, no price swings"). Bank multi-currency = Stripe-native (non-USD Connect account converts USD on payout, no code). Adversarial verify: SOUND 5/5 (correct 6-decimal units, no silent SOL downgrade, SOL+fiat paths unchanged, exactly-once == SOL). tsc clean, deployed.
  - **Standing money-hardening item flagged (PRE-EXISTING, affects SOL too, NOT a 4.5 regression):** if the DB write of `payout_tx`/`payout_released` fails AFTER a successful on-chain crypto send (both fields fail atomically → retry-payout's `if (claimed.payout_tx)` guard can't catch it), a later retry double-pays. Fix candidates: write payout_tx optimistically BEFORE the send, or a payout-attempts ledger keyed on (order,seller) as a natural dedupe. Card/Connect rails are safe (Stripe idempotency key). Spawned a task chip for this.
  - Phase 4 remaining: 4.6 Moov card reader checkout, 4.9 Stripe Tax (flag, Judah nexus), 4.10 1099-K (flag, Judah entity).
- **2026-07-04 (cont.)** — **4.6 Moov card: ONLINE rail verified DONE, card-present deferred.** Assessed the existing Moov build (no new code needed): `moov.ts` + `/api/moov/{card-token,charge,webhook}` + `<MoovCardForm>` are complete and wired into the checkout CARD tab behind `NEXT_PUBLIC_MOOV_ENABLED`. Read `/api/moov/charge` this session — sound: server-priced, authed+wallet-owned+rate-limited, idempotency-keyed, `waitForRailResponse`, fulfills ONLY on `completed` via the SHARED `fulfillPurchase` (same order/Tally-mint/payout path as Stripe; seller paid from platform wallet by payout-on-delivery = 4.3 fiat + 4.5 crypto). So "Moov card → same state machine → mint + payout" is met for ONLINE card. **Physical card-present (tap-to-pay/terminal) DEFERRED to Phase 9** — the Moov lib has no terminal API; needs Moov's Terminal/Tap-to-Pay product + device entitlements + the native Capacitor shell. Did NOT write inert hardware code. Judah enablement (also in [[launch-integrations-status]]): paste MOOV_WEBHOOK_SECRET, confirm capabilities + visby.me allowlist, sandbox charge test, set NEXT_PUBLIC_MOOV_ENABLED=1; sandbox keys → reconcile before real money.
  - **Next: 4.9 Stripe Tax + 4.10 1099-K** — both "build behind flags, enable when Judah confirms nexus/entity" → BUILDABLE now (only enabling is gated). Then Phase 4 Gate → Phase 5.
- **2026-07-04 (cont.)** — **4.9 Stripe Tax + 4.10 1099-K DONE → PHASE 4 COMPLETE (10/10).**
  - **4.9 (flag-dark, STRIPE_TAX_ENABLED):** `src/lib/tax.ts` (calculateOrderTax via stripe.tax.calculations.create, fail-open to $0 on error/incomplete address) → wired into `/api/stripe/payment-intent` (charges price+tax, PI metadata tax_cents+tax_calc_id) → webhook finalizes via stripe.tax.transactions.createFromCalculation on succeeded (best-effort, idempotent by `order:<pi.id>` reference) → checkout UI Sales-tax line + tax-inclusive total. Adversarial verify: **flag-off = byte-identical no-op** (verified) so safe to deploy dark; ONE flag-on defect found+FIXED (CardPayForm pay button showed tax-EXCLUSIVE price while PI charged price+tax → added a payAmount prop = price+tax). Enable only after Judah registers to collect tax. Follow-up: extend tax to ACH+Moov rails (card-only now).
  - **4.10 (1099-K):** overwhelmingly Stripe-Connect-native — 4.3 Express onboarding already collects seller tax info; Stripe files forms for bank-payout connected accounts once Judah enables it in the Connect dashboard (+ entity/EIN). Built the app-side companion: `src/lib/earnings.ts` + `/api/seller/earnings` (authed read-only, realized earnings by year = the data behind a 1099-K). CAVEAT: crypto-payout (SOL/USDC) sellers NOT covered by Stripe's Connect 1099-K → separate entity-gated Visby-issued filing (Judah decision).
  - **PHASE 4 GATE STATUS:** all rails (Stripe card, ACH, Moov, crypto SOL/USDC, Stripe Connect fiat) settle via the SHARED fulfill/order/payout machine, non-custodial, with reconciliation (4.8 cron). Flag-dark items (ACH, Moov, Tax) need Judah enablement before real money. Money hardening item spawned (crypto write-after-send double-pay). **→ Phase 5 (SDK & merchant experience) next.**
- **2026-07-04 (cont.)** — **Phase 5 started: 5.1 + 5.2 + 5.3 (merchant dashboard completion) DONE.** Recon (Explore agent) mapped the SDK/merchant surface: the hard backend (`sdk_orders` w/ fee_bps+platform_fee_usd+merchant_net_usd stored at checkout, webhook retry columns, `redeliverPendingSdkWebhooks` cron) was solid; the dashboard (`merchant/page.tsx`) only had a filtered webhook-status list, no re-send button, no revenue view.
  - **5.1 orders list + 5.3 revenue (Sonnet subagent):** new `/api/merchant/orders` (Privy-authed via owner_wallet+callerOwnsWallet+merchant-ownership re-check, missing-schema-safe) → orders (50, full cols) + summary (gross/platform_fee/merchant_net over paid+minted, by-status counts). Dashboard: RevenueCard (Gross/Platform fees/Your net + status breakdown) + OrdersCard (per-order badge/price/net/date/webhook indicator).
  - **5.2 manual re-send (I built the webhook-critical part):** `redeliverSdkOrderNow(orderId)` added to sdk-webhook-redelivery.ts (reuses buildSdkWebhookEvent+deliverSdkWebhook+state-update; one-shot, no cap/reschedule) + `/api/merchant/redeliver` (Privy-authed, one-query order-ownership verify, rate-limited). The cron `/api/sdk/redeliver-webhooks` stays CRON_SECRET-gated/batch — couldn't be exposed to merchants (wrong auth+scope). Per-row "Re-send webhook" button on undelivered terminal orders; flips badge only on real delivery.
  - tsc clean; /merchant hydrates clean (walletconnect registry-fetch console errors are pre-existing/benign in the sandboxed preview). Deployed.
  - **Phase 5 remaining:** 5.4 extension Privy auth (MISSING, design-heavy — MV3 popup + Privy needs chrome.identity/auth-relay), 5.5 extension checkout session (PARTIAL — needs a publishable-key-safe session-creation mechanism; extension can't hold sk_visby_), 5.6 merchant payout settlement (MISSING code — SDK side has ZERO payout code; buyer funds land in Visby treasury, merchant_net never disbursed; scaffolding buildable but go-live blocked on merchant onboarding/compliance), 5.7 SDK docs polish (DONE-ish — add crypto path + error reference + image_url). 5.4/5.5 need design decisions; 5.6 go-live blocked; 5.7 incremental.
- **2026-07-04 (cont.)** — **5.7 SDK docs polish DONE** (Sonnet subagent, verified accurate). `src/app/sdk/page.tsx` += image_url in examples, a "pay on-chain" card (accurate to settleSdkOrderCrypto: hosted-checkout-only, no direct merchant crypto API, pay_method='crypto'), and an Error-reference table with REAL codes/strings from checkout/route.ts (401 bad key, 400 field validation, 422 brand-registry serial_rejected [dynamic reason, not a fixed literal], 429 rate-limit, 503 schema-missing). tsc clean; /sdk renders all sections; deployed.
  - **Judah answered the two design calls + I built 5.4:**
    - **5.4 = auth-relay (Judah's pick), BUILT.** MV3 popups can't run Privy's web SDK, so: `src/app/extension-auth/page.tsx` (in the app's Privy provider) logs in then relays the Privy token to the extension via `chrome.runtime.sendMessage(extId, {type:'visbyAuth', payload:{token,wallet}})`; extension manifest gets `externally_connectable` (visby.me/app.visby.me/localhost); background.js adds an ORIGIN-VALIDATED `onMessageExternal`→setAuth; popup.js sign-in opens `/extension-auth?ext=<runtime.id>` + live-refreshes on storage change. **Also fixed a real pre-existing break:** the extension's `VISBY_ORIGIN` was `https://app.visby.me` which DOES NOT RESOLVE (app is at visby.me) → repointed config.js + background.js to `https://visby.me` (whole extension was talking to a dead domain). tsc clean; relay page renders; extension JS + manifest validated. E2E token-handoff needs the unpublished extension loaded in Chrome = manual Judah step.
    - **5.5 REFRAMED by Judah:** "the extension exists for non-partner sites so you can check out quickly anyway... whatever works for just burner card autofilling." So the extension's real purpose = **burner-card autofill on ANY site** (all-inclusive pay-anywhere), NOT partner SDK checkout sessions. That = the **burner card = regulated card ISSUING**, which is **PARKED → Phase 10** (Lithic/Stripe Issuing/Moov Issuing decision). 5.5-as-written is obsolete; don't build a partner-checkout-session. See [[visby-superapp-plan]] (VisbyPay universal pay) + the Phase-10 parked burner-card note.
  - **SESSION-END STATE (Phase 5): DONE 5.1/5.2/5.3/5.4/5.7 (+ 4.7/4.8 earlier). Only 5.6 remains** (5.5 parked→Phase 10). **NEXT SESSION START HERE: 5.6 merchant payout** — crypto USDC to `merchants.merchant_wallet` (reusing 4.5's sendUsdcFromAuthority; NOT onboarding-blocked for the crypto path), a MONEY PATH → build+adversarial-verify+deploy as ONE unit in a fresh session (avoid the half-built-money-path risk this very session recovered from). Needs: payout columns on sdk_orders (payout_status/payout_tx/payout_next_attempt_at), a payout call appended to mintAndDeliver in sdk-settle.ts (idempotent, after mint), + a retry sweep like redeliver-webhooks.
- **2026-07-04 (cont.)** — Judah: "enable whatever you need through compositio. keep going. also bear in mind it should have email and password. not just password at every new login."
  - **Login = EMAIL + PASSWORD (feedback, DONE):** the hard login gate (password-gate.tsx) showed only a password field → felt like "just password." FIXED: the account's Privy-verified email is now shown read-only (prefilled) above the password field on BOTH the set-password and welcome-back screens; headings reframed ("Welcome back — Sign in with your email and password"). Email isn't re-entered (Privy already verified it for the session). tsc clean, app hydrates clean (this is the whole-app-wrapping component — did the browser render check per the crash lesson). Deployed.
  - **ACH ENABLED via Composio (test mode):** confirmed STRIPE_SECRET_KEY is sk_TEST (safe). (1) Added `payment_intent.payment_failed` + `payment_intent.canceled` to the test webhook `we_1ToExMJzcGyqGCv5tAhczbdw` (visby.me/api/stripe/webhook) DIRECTLY via the Stripe API using the local test key (Composio's Stripe toolkit has NO webhook-update tool + is connected to the LIVE account, not test). (2) Set `NEXT_PUBLIC_ACH_ENABLED=1` on Vercel (Composio VERCEL_ADD_ENVIRONMENT_VARIABLE — NOTE: the project NAME 403'd; the project ID `prj_HcafGXsng1YQIyxunUOoEGOfGZNo` worked) + added to .env.local. Takes effect on next deploy. So ACH is now live-in-test. **Stripe Tax left DARK** — the STRIPE_TAX_ENABLED flag does nothing without Stripe-dashboard Tax registration (Judah's nexus decision); enabling it now would just fail-open to $0. **Moov still needs MOOV_WEBHOOK_SECRET** (a value I don't have). See [[launch-integrations-status]].
  - **5.6 merchant payout BUILT this session** (money path, cron-sweep design — decoupled from the buyer checkout so the mint hot path is UNTOUCHED, no serverless fire-and-forget risk): migration_sdk_merchant_payout.sql (merchant_payout_status/tx/at/next_attempt_at/last_error on sdk_orders, applied live) + `src/lib/sdk-merchant-payout.ts` (attemptMerchantPayout = CAS claim pending|failed→processing BEFORE sendUsdcFromAuthority(merchant_wallet, merchant_net_usd) [1:1 USD→USDC, no oracle], then paid+tx / failed+retry; runDueMerchantPayouts sweep over status='minted' + payout pending|failed + due) + `/api/sdk/pay-merchants` cron (CRON_SECRET-gated, timing-safe) + vercel.json daily 10:00 UTC. Exactly-once via CAS; stuck 'processing' (crash mid-send) is NOT auto-reclaimed (avoids double-pay) → manual reconcile (same standing crypto-payout risk class as marketplace). **Adversarially verified SOUND 6/6** (CAS exactly-once, 1:1 USD→USDC, minted-only+net>0+wallet guards, stuck-processing safe, cron auth fails-closed timing-safe, sdk-settle.ts BYTE-UNCHANGED). Deployed. Backlog (non-blocking): (a) confirm-timeout→retry can re-send on-chain = same standing crypto-payout risk → the spawned hardening chip [[transfers-toctou-followup]]-adjacent now ALSO scopes `src/lib/sdk-merchant-payout.ts` (not just payout.ts); (b) no alert for a stuck-'processing' row (add a "processing older than N min" captureMessage later).
  - **PHASE 5 EFFECTIVELY COMPLETE** — 5.1/5.2/5.3/5.4/5.6/5.7 done+deployed; 5.5 parked→Phase 10 (burner card / regulated issuer). Manual/gated leftovers: extension auth-relay e2e (load unpacked in Chrome, Judah), 5.5's real form (Phase-10 card-issuer decision). **NEXT: Phase 6 (Fraud & trust stack) — Civic KYC swap, Chainalysis screening, dispute evidence, chargeback playbook, NFC.**
- **2026-07-04 (cont.)** — Judah "keep going, don't stop unless questions/can't." **Phase 6 started.** Blueprint numbering: 6.1 Civic KYC (BLOCKED — needs Judah's Civic account+keys), 6.2 multi-wallet KYC propagation (part of KYC, blocked), 6.3 flip NEXT_PUBLIC_KYC_REQUIRED (depends on 6.1/6.2), 6.4 Chainalysis screening (BLOCKED — needs Chainalysis account), 6.5 dispute evidence (buildable), 6.6 chargeback bundle export (buildable), 6.7 NFC (after QR/hardware), 6.8 suspend+ban (DONE). So 6.1/6.2/6.4 need Judah's external accounts; 6.5/6.6 are the buildable-now items.
  - **6.5 dispute evidence DONE** (Sonnet subagent + my authz review + migration): `dispute_evidence` table (live) + `/api/disputes/evidence` (POST multipart→'dispute-evidence' bucket+row; GET evidence + derived proof-of-delivery from orders.tracking_*). AUTHZ reviewed sound: getAuthedContext + ctx.wallets.includes(wallet) + dispute buyer/seller OR finance/moderator admin (else 403); no evidence on resolved disputes (409); 10MB + image/pdf allowlist; rate-limited. UI: dispute-panel evidence section + admin/disputes evidence viewer. tsc clean, 401 unauth, pages compile. Deployed. **Follow-up: PUBLIC bucket (like item-images) → private+signed-URLs is a hardening step for sensitive evidence.**
  - **6.6 chargeback bundle export DONE:** `/api/admin/chargeback-bundle?order_id=` (admin-authed finance/moderator) compiles transaction + item authenticity (NFT/serial) + delivery proof (carrier/tracking/dates) + on-chain provenance (ownership_history tx hashes) + dispute + 6.5 evidence into one issuer-ready bundle. Printable admin page `/admin/chargeback/[orderId]` (Print/Save-as-PDF, print CSS) linked from each admin dispute card. tsc clean, API 401 unauth, pages compile. Deployed.
  - **Phase 7 (UX) started — 7.1 first-run onboarding DONE:** `first-run-onboarding.tsx` — one-time 3-slide plain-English intro (Welcome / what a Tally is [no crypto jargon] / how paying works), localStorage-gated (shows once/device), skippable, on home page. Preview-verified (renders, dismiss persists flag, home hydrates behind it). tsc clean, deployed. **7.11 fee transparency DONE:** buyer already sees the full checkout total (incl. 4.9 tax line + tax-inclusive pay button); relist flow already showed the fee breakdown; ADDED it to the mint "Mint & List" flow ("Visby fee (9%) −$X · You net ~$Y before shipping" under the price input). Deployed. **Phase 7 progress:** DONE = 7.1 onboarding, 7.11 fee transparency, 7.10 OG images (item + profile next/og ImageResponse, verified generating PNGs). ALREADY-BUILT (audited): 7.5 order timeline (full spine on order page), 7.9 seller analytics (SellerAnalytics on wallet page). **7.6 help center DONE** (Sonnet subagent + my review/fix): support_requests table (live) + /api/support/submit (signed-out-ok, dual rate-limit, escapeHtml injection-guard, fire-and-forget email) + /help FAQ+contact page + header-menu Help link. **CAUGHT+FIXED a subagent factual error**: it defined "Tally" as "account balance/identity" (WRONG — Tally = the item's provenance/authenticity cert per design.md/tally-explainer.tsx); rewrote the FAQ answer correctly. Endpoint reviewed sound; migration applied; deployed. **7.7 returns = ALREADY BUILT** (dispute-panel opens a 'return'-kind dispute via POST /api/disputes → feeds the dispute/refund system). **7.4 address book DONE** (Sonnet subagent, my review): shipping_addresses table (live) + /api/buyer/addresses (CRUD, callerOwnsWallet, validation, 20-cap) + `<AddressBook>` on /settings. PURELY ADDITIVE — set-default mirrors into profiles.ship_to (the field checkout reads); **checkout-modal.tsx + ship-to/route.ts verified BYTE-UNTOUCHED**. Deployed. **PHASE 7 STATE: DONE = 7.1, 7.4, 7.5, 7.6, 7.7, 7.9, 7.10, 7.11 (8 of 11). Remaining: 7.2 empty/error-states sweep (broad polish), 7.3 offers flow (PARTIAL — preset-composer has an offer slider in messaging; the accept→checkout-at-offered-price leg needs completing), 7.8 notification prefs (THIN — only social types [followers/following] are gate-able; all order/money/dispute/security notifications are transactional & must NEVER be gated; low-value, do carefully). Self-heal auto-email-triage stays out of scope — injection-sensitive [[auto-bugfix-email-feature]].
- **2026-07-04 (cont.)** — Judah decisions + a prod bug fix + onboarding polish:
  - **Phase 6 vendor decisions:** KYC = **Stripe Identity** (Civic shredded — already-integrated, ~$1.50/verif, first 50 free, no new signup; needs Judah to enable Identity in the Stripe dashboard). Sanctions = **Chainalysis FREE Sanctions API** (skip the expensive enterprise product). See [[launch-integrations-status]] + blueprint 6.1/6.4.
  - **"Can we get by without them for MVP?"** → YES for a closed beta: KYC is already a flag (off by default) + Stripe Connect already identity-verifies fiat-payout sellers; sanctions screening is free + belongs "before public crypto." Attorney sign-off on the non-custodial/KYC posture before real money at scale (already a non-code blocker).
  - **Judah: "keep stripe verifications"** → BUILD Stripe Identity KYC (6.1/6.2/6.3), dark-flagged. Recon done (NOT yet coded — interrupted by the email bug below): `src/lib/kyc.ts` is the contract to KEEP (KycStatus, kycRequired() flag, requireKycForSaleAny() sell-gate already wired into mint+listing, setKycStatus). Swap plan: `persona.ts`→a stripe-identity adapter (verificationSessions.create + Stripe webhook verify), rewrite `/api/kyc/start` (create session, return client_secret for Stripe.js verifyIdentity) + `/api/kyc/webhook` (map identity.verification_session.verified→approved / requires_input→review, fail-closed), swap `kyc-verify.tsx` client to Stripe.js. `kyc_verifications` table is generic (provider/inquiry_id/status/raw) → NO migration, use provider='stripe_identity', inquiry_id=vs_id. **BUILT + DEPLOYED (dark) 2026-07-04:** `src/lib/stripe-identity.ts` + rewritten `/api/kyc/{start,webhook}` (webhook re-fetches the session from Stripe before approving = fail-closed; 401 on unauth/no-sig). Sell gates + kyc.ts + kyc-verify.tsx unchanged. 6.2 multi-wallet propagation ALREADY handled by kyc.ts (requireKycForSaleAny/getKycStatusForUser = per-person). tsc clean. Enable = the 3 Judah steps in action-item #5.
  - **PASSWORD-RESET EMAIL BUG FIXED (prod):** Judah never got the reset email. Root cause: Vercel `RESEND_API_KEY` was STALE (old 07-01 key, revoked) while .env.local had the current restricted send-key; local sends worked, prod silently skipped (fire-and-forget + always-ok route = invisible fail). Diagnosed: config+code+Privy resolveEmail all fine locally (test send 200 to Judah). FIX: synced Vercel RESEND_API_KEY + EMAIL_FROM to the working values via Composio (activates next deploy). **The working key was printed in-session → tell Judah to ROTATE it in Resend as hygiene.** See [[launch-integrations-status]].
  - **Onboarding v2 (7.1):** per Judah — expanded 3→5 slides (Welcome / Tally / Buying / Selling / You're all set), warmer + less-AI copy, em-dashes→commas (user-facing text), and the Tally slide now says "Tallys are NFTs used as authenticators, so they can't be faked, copied, or tampered with." SEEN_KEY bumped v1→v2 so it re-shows. Preview-verified. Deployed.
  - **PHASE 6 STATE: BUILDABLE items done (6.5, 6.6, + 6.8 earlier). BLOCKED on Judah's external accounts:** 6.1 Civic KYC + 6.2 multi-wallet KYC propagation + 6.3 flip NEXT_PUBLIC_KYC_REQUIRED all need a **Civic account + API keys** (the built-but-dormant Persona flow's fail-closed webhook architecture gets kept, vendor swapped to Civic); 6.4 Chainalysis wallet screening needs a **Chainalysis account + API key**. 6.7 NFC is after-QR/hardware. So Phase 6's fraud CORE (KYC + sanctions screening) is gated on two Judah signups — surfaced to him via AskUserQuestion this session. Once keys land I build the Civic adapter (swap Persona) + Chainalysis screening (fail-closed on sanctioned, admin review queue) + flip the KYC flag.
- **2026-07-05** — Judah: password reset "still not working" → then "Password reset worked, keep going."
  - **PASSWORD-RESET ROOT CAUSE + FINAL FIX:** the earlier Vercel `RESEND_API_KEY` sync didn't unblock Judah's retry. Ground-truthed via Composio Resend logs: visby.me domain VERIFIED + my direct test DELIVERED to Judah's Gmail, but ZERO reset emails EVER in the account (prod was rejecting sends on a stale key → invisible fail, since the route always returns ok + fire-and-forget). Unblocked Judah DIRECTLY: wrote a temp password to `account_security.password_hash` (scrypt, matching account-password.ts format) via a service-role script → he signed in + reset it → CONFIRMED WORKING. Prod reset now uses the synced key. **Judah still to ROTATE the Resend key** (printed in-session). See [[launch-integrations-status]].
  - **PHASE 8 STARTED — 8.1 data-export + 8.2 account-deletion DONE + DEPLOYED (commit de7b50e):** Phase 7 gate judged met (onboarding/help/fee-transparency/order-timeline shipped; 7.2/7.3/7.8 = non-gating polish) → advanced to Phase 8.
    - `/api/account/export` (POST, authed, ban-gated): JSON copy across 18 tables, EXPLICIT safe-column allowlists ONLY (never select('*') → a future secret column can't leak); excludes password_hash/reset tokens/stripe ids/push-token value/device fingerprint+ip/kyc `raw`; each fetch fail-soft; Privy email resolved fail-soft (not stored in our DB).
    - `/api/account/delete` (POST, authed, typed-"DELETE" confirm): obligation guard FAILS CLOSED — blocks on in-flight (paid/shipped) orders, delivered orders still in escrow (payout_released=false), and open/under_review disputes; 503 on any guard read error. ANONYMIZE profiles (null PII, account_status='deleted', deleted_at) + HARD-DELETE pure-PII tables (best-effort, logged); RETAIN orders/transfers/disputes/dispute_evidence/reviews/kyc/items/ownership_history (tax/AML/provenance). messages + payment_requests scoped to the user's OWN authored rows.
    - `migration_account_deletion.sql` APPLIED (Composio SUPABASE_APPLY_A_MIGRATION, arg is `ref` not project_id): +deleted_at, widened profiles.account_status CHECK to allow 'deleted'.
    - `account-status.ts`: getWorstStatus maps 'deleted'→'banned' (ONE line) → a deleted account is locked out through the EXISTING ban gate everywhere, no new enforcement surface.
    - Settings UI (Sonnet subagent + my review): "Download my data" + "Delete account" (typed-DELETE confirm, privy.logout()+redirect on success, 409-obligations message). tsc clean.
  - **ADVERSARIAL VERIFICATION WORKFLOW (6 lenses / 16 agents) — 7 confirmed defects, ALL FIXED pre-deploy:** (HIGH) messages OR-delete wiped the COUNTERPARTY's authored messages → scoped to from_wallet; (MED) payment_requests PII table missed → added (scoped to requester_wallet, avoiding the same over-delete trap); (HIGH) obligation guard missed delivered+escrow orders → added that query; + FOUR pre-existing ban-gate HOLES the 'deleted'→banned mapping inherits, now closed to match transfer/prepare: **transfer/confirm, tally/transfer, payout POST, account/export** reject banned/deleted (403). delete route deliberately NOT ban-gated (so a partially-failed deletion can be re-run). 1 refuted, 1 uncertain (orIn() wallet-charset assumption — non-exploitable: Privy mints base58/0x-hex, delimiter-free; latent hardening only). tsc clean.
  - **8.3 legal routes DONE (Sonnet subagent + my review):** existing legal system (legal_documents table kind=terms|privacy, /api/legal read, /api/admin/legal PDF-upload, LegalShell, /legal/* pages) EXTENDED to add **Acceptable Use** (`acceptable_use`) + **Seller Agreement** (`seller_agreement`). migration widened the kind CHECK (Composio; robust drop-all-check-constraints then re-add). New /legal/acceptable-use + /legal/seller-agreement pages; /api/legal + /api/admin/legal allowlists widened; LegalShell union widened; links added to legal-footer + header-menu; admin/legal upload UI gained 2 UploadRows. tsc clean, deployed. **All 4 legal ROUTES now live (Phase 8 gate) — CONTENT is Judah's: upload the 4 PDFs from counsel via /admin/legal (pages show "being finalized" until then).**
  - **PHASE 8 REMAINING:** 8.4 Resend catch-all aliases (Judah DNS), 8.5 AML record-keeping (deferred with sanctions). Phase 7 tail (7.2/7.3/7.8) still open as polish.
  - **8.3 CONFIRMED LIVE** (visby.me): /api/legal?kind=seller_agreement → {"url":null} (new code) + /legal/{terms,acceptable-use,seller-agreement} all 200. (First poll 404'd ~4 min = deploy lag, not a failure.)
  - **9.3 PWA = DONE (audited, already built):** manifest.ts (complete: standalone, 192/512 any+maskable icons, theme/bg color), public/sw.js (network-passthrough, satisfies install criteria), `<PwaRegister/>` MOUNTED in layout.tsx (registers /sw.js), apple-touch-icon + appleWebApp metadata. Installable. Lighthouse ≥90 = a Judah/tooling verify. The old superapp-memory "missing manifest" worry is RESOLVED. **Phase 9.1 (Capacitor) + 9.2 (extension store) are Judah/store-submission gated.**
  - **NO TEST RUNNER yet** (Phase 11): package.json has no test script, no vitest/jest, no *.test.ts. fees.ts + order-state-machine.ts are PURE (no imports) → trivial vitest targets when Phase 11 starts.
  - **PHASE 10 design sweep STARTED (audit + safe fixes):** 15-route fan-out design audit vs design.md (28 agents, each finding re-verified to drop brand-hex/functional-color/frozen-logo/price() false positives) → **119 confirmed violations (12 high / 60 med / 47 low)** across 29 files → written to `PHASE10_DESIGN_AUDIT.md` (the review artifact, grouped by file with per-item fix). FIXED the SAFE high-severity ones (Sonnet subagent + my diff review, tsc clean, deployed): 6 hardcoded rgba(0,0,0,.5) modal/sheet scrims → var(--modal-scrim) (profile, header-menu, home filters popup, dashboard pay-sheet, pay-request ×2); glass-in-glass override removal in payment-methods-manager + tally-wallets dropdowns; header-menu trigger glass→surface(); buy-crypto invalid `var(--ok)12`-style CSS → var(--ok-soft); profile SOL chip Solana-gradient+#fff → surface()+token. **DEFERRED (bigger/careful, in PHASE10_DESIGN_AUDIT.md):** checkout-modal.tsx (4 high — MONEY-PATH buy flow, needs an isolated type-scale/spacing/glass-nesting rewrite WITH checkout testing, NOT a bulk edit) + pay-request Pay|Request bespoke tab → tabSlider() + the 107 med/low backlog. Routes /marketplace + /login audited CLEAN.
  - **NEXT:** Phase 10 tail (checkout-modal careful refactor + pay-request tabSlider + med/low backlog — Judah prioritizes from the artifact) → Phase 11 = TEST EVERYTHING (no runner yet; fees.ts + order-state-machine.ts are pure vitest targets).
---
