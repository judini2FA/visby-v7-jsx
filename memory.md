...
- **2026-07-04 (cont.)** — Judah "keep going, don't stop unless questions/can't." **Phase 6 started.** Blueprint numbering: 6.1 Civic KYC (BLOCKED — needs Judah's Civic account+keys), 6.2 multi-wallet KYC propagation (part of KYC, blocked), 6.3 flip NEXT_PUBLIC_KYC_REQUIRED (depends on 6.1/6.2), 6.4 Chainalysis screening (BLOCKED — needs Chainalysis account), 6.5 dispute evidence (buildable), 6.6 chargeback bundle export (buildable), 6.7 NFC (after QR/hardware), 6.8 suspend+ban (DONE). So 6.1/6.2/6.4 need Judah's external accounts; 6.5/6.6 are the buildable-now items.
  ...
  - **Phase 7 (UX) started — 7.1 first-run onboarding DONE:** `first-run-onboarding.tsx` — one-time 3-slide plain-English intro (Welcome / what a Tally is [no crypto jargon] / how paying works), localStorage-gated (shows once/device), skippable, on home page. Preview-verified (renders, dismiss persists flag, home hydrates behind it). tsc clean, deployed. **7.11 fee transparency DONE:** buyer already sees the full checkout total (incl. 4.9 tax line + tax-inclusive pay button); relist flow already showed the fee breakdown; ADDED it to the mint "Mint & List" flow ("Visby fee (9%) −$X · You net ~$Y before shipping" under the price input). Deployed. Phase 7 remaining (all buildable, no external deps): 7.2 empty/error states, 7.3 offers flow, 7.4 address book, 7.5 order-tracking timeline, 7.6 help center, 7.7 returns/refunds, 7.8 notif prefs, 7.9 seller analytics, 7.10 OG images.
