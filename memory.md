...
- **2026-07-04 (cont.)** — **5.7 SDK docs polish DONE** (Sonnet subagent, verified accurate). `src/app/sdk/page.tsx` += image_url in examples, a "pay on-chain" card (accurate to settleSdkOrderCrypto: hosted-checkout-only, no direct merchant crypto API, pay_method='crypto'), and an Error-reference table with REAL codes/strings from checkout/route.ts (401 bad key, 400 field validation, 422 brand-registry serial_rejected [dynamic reason, not a fixed literal], 429 rate-limit, 503 schema-missing). tsc clean; /sdk renders all sections; deployed.
  - **SESSION-END STATE (Phase 5 partial): DONE 5.1/5.2/5.3/5.7 (+ 4.7/4.8 earlier). REMAINING = 5.4, 5.5, 5.6.** Deliberately NOT built autonomously this session: 5.4 (extension Privy-in-popup) + 5.5 (extension publishable-key-safe checkout session) need a DESIGN DECISION and the extension is unpublished (can't e2e test) — good candidates to ask Judah or do in a focused session. 5.6 (merchant payout — crypto USDC to merchants.merchant_wallet, reusing 4.5's sendUsdcFromAuthority; NOT onboarding-blocked for the crypto path) is a MONEY PATH → build+adversarial-verify+deploy as ONE unit in a fresh session (avoid the half-built-money-path risk this very session opened by recovering from). NEXT SESSION START HERE: 5.6 crypto merchant payout, or 5.4/5.5 after a design call.
---
